...
| `CODCHAT_LOOKUP_API_TOKEN` | Web + API — same value in both | Yes to serve CodChat order-status lookups. commerce-api has no public address (see the note above this table), so CodChat actually calls the web app's `/api/codchat/order-status` relay, which checks this same token and forwards the request over the private connection the web app already has to commerce-api at `COMMERCE_API_BASE_URL`. Run `infra/cloudshell/scripts/23-set-codchat-secret.sh` — it generates the token, writes it into both the api and web secrets, and attaches it to both running ECS task definitions in one step; do not create it by hand, since a value dropped into the JSON secret alone is not enough — every individual env var needs its own explicit wiring into the task definition, which this script does for you. It prints the token once at the end; paste that exact value into CodCRM's Data sources screen as the connector's bearer credential. Absent on the API, that route is not registered at all; absent on the web app, the relay always answers 401 — skip the script entirely if CodChat is not being wired up yet |
| `SITE_BASE_URL` | API (plain env from `SITE_URL`) | Set by `09-create-ecs.sh` / CloudFront — do not duplicate in a secret. Also what `/v1/codchat/order-status` resolves GWG's tenant/account/store scope from |
