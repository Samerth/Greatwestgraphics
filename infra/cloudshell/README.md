...
| 25 | `25-set-openai-secret.sh` | Reads Great West Graphics' OpenAI key with a silent prompt, writes it into the web JSON secret, sets `STUDIO_AI_PROVIDER=openai` on the web task and rolls the service. Switches the Design Studio's AI Art panel (and the row 61 background remover) onto the paid model. See docs/AI_PROVIDER_OPENAI.md |
